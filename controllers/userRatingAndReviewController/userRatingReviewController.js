...
exports.createRatingReview = async (req, res) => {
  try {
    ...
    let { rating, review, itemId, sizeBought } = req.body;
    const { userId } = req.user;

    // Validate required fields
    if (!rating || !itemId) {
      return res.status(400).json(apiResponse(400, false, "rating and itemId are required"));
    }
    ...
    const existingReview = await UserRatingReview.findOne({ userId, itemId });
    if (existingReview) {
...
exports.deleteRatingReview = async (req, res) => {
  try {
    ...
    const { itemId } = existingReview;

    // Delete review
    await UserRatingReview.findByIdAndDelete(reviewId);

    // Recalculate average rating for the item
    const remainingReviews = await UserRatingReview.find({ itemId }).select("rating").lean();
...
exports.getRatingsAndReviewsByItemId = async (req, res) => {
  try {
    const { itemId } = req.params;

    if (!itemId) {
      return res.status(400).json(apiResponse(400, false, "itemId is required in params"));
    }

    const reviews = await UserRatingReview.find({ itemId })
      .populate("userId", "name")
      .sort({ createdAt: -1 });

    if (!reviews || reviews.length === 0) {
      return res.status(404).json(apiResponse(404, false, "Reviews not found for this item"));
    }
    ...
    const totalReview = reviews.filter((review) => review.review && review.review.trim() !== "").length;
